...
bool PacketReceipt::validate_link_proof(const Bytes& proof, const Link& link, const Packet& proof_packet) {
	...
	if (true || proof.size() == EXPL_LENGTH) {
		...
		if (proof_hash == _object->_hash) {
			...
			// which was disabled (replaced with `if (false)`). That meant
			// DIRECT-via-link delivery proofs were received, hash-matched,
			// signature-extracted, and then the delivery callback was
			// silently dropped. Restore the link.validate call so the
			// PacketReceipt's _delivered_callback fires when the proof's
			// link signature checks out — matches python RNS's
			// PacketReceipt.validate_link_proof.
